...
#
# Expected values are generated by a one-time offline run of the OMFIT
# `OMFITgeqdsk` reader (raw parser + fluxSurfaces tracer) on the same
# ITER_test.eqdsk file.  The TokaMaker reader was adapted from OMFIT, so:
...
#   - Derived flux-surface quantities (q, geometry, li, betas) are from
#     OMFIT's fluxSurfaces; per-key tolerances are relaxed where the
#     TokaMaker implementation uses a slightly different algorithm
#     (e.g. OMFIT uses a 4-point triangularity definition, we use a
#     parabolic-fit one; edge resampling differs; etc.).
#   - j_tor_averaged (<Jt/R>/<1/R>), j_tor_averaged_direct
#     (-(p'<R> + FF'<1/R>/mu0)*(2pi)^exp_Bp, the TokaMaker default),
#     and q_profile are each sampled at five interior psi_N positions
#     with very tight (0.5%) tolerance.  These are the core analytic
#     Grad-Shafranov outputs used by bootstrap and reconstruction,
#     and alignment with OMFIT is ~0.004% RMS for the standard j_tor
#     and ~0.01% for j_tor_direct when the machinery is healthy.
# Regenerate with tests/physics/generate_eqdsk_expected.py in an env
# with scipy<1.14 / numpy<2.0 (e.g. the `omfit_env` conda env).
# -----------------------------------------------------------------------
eqdsk_gfile_expected_dict = {
    # --- Raw scalars (OMFIT raw parser) ---
    'NW':       64.0,
    'NH':       64.0,
    'RDIM':     4.77959814,
    'ZDIM':     10.19414,
    ...
    'BCENTR':   5.28076054,
    'CURRENT':  15599996.7,
    # --- 1-D profiles: spot values + sums ---
    'FPOL_0':       35.0621486,
    'FPOL_mid':     33.2261739,
    'FPOL_end':     32.8600001,
    'FPOL_sum':     2144.3620783,
    'FPOL_len':     64,
    'PRES_0':       620000.23,
    'PRES_mid':     231873.572,
    'PRES_end':     1.49180634,
    'PRES_sum':     16598748.7375183,
    'PRES_len':     64,
    'FFPRIM_0':     11.549964,
    'FFPRIM_mid':   4.70984576,
    'FFPRIM_sum':   333.5512928898,
    'FFPRIM_len':   64,
    'PPRIME_0':     53875.4763,
    'PPRIME_mid':   50294.7342,
    'PPRIME_end':   207.354147,
    'PPRIME_sum':   2744050.799227,
    'PPRIME_len':   64,
    'QPSI_0':       0.821330888,
    'QPSI_mid':     1.10105825,
    'QPSI_end':     4.81428126,
    'QPSI_sum':     87.770020125,
    'QPSI_len':     64,
    # --- 2-D PSIRZ spot values ---
    'PSIRZ_min':    -3.37288968,
    'PSIRZ_max':    27.7856972,
    'PSIRZ_sum':    56218.66018626,
    # --- Boundary and limiter ---
    'RBBBS_min':    4.24079094,
    'RBBBS_max':    8.20438613,
    'RBBBS_len':    64,
    'ZBBBS_min':    -3.21666299,
    'ZBBBS_max':    4.01026064,
    'ZBBBS_len':    64,
    'RLIM_min':     4.0455,
    'RLIM_max':     8.39058922,
    'RLIM_len':     153,
    ...
    'ZLIM_max':     4.7115,
    'ZLIM_len':     153,
    # --- Derived FSA quantities (OMFIT fluxSurfaces tracer) ---
    'Ip_enclosed_edge':    -15588938.4950752,   # ~CURRENT; sign depends on COCOS
    'q_axis':              -0.8132830481240,
    'kappa_edge':          1.8730775493884,
    'delta_edge':          0.4801753788544,
    'a_edge':              1.9837833388564,
    'R_geo_edge':          6.2222755620148,
    'vol_edge':            824.1868582695,
    'cxArea_edge':         21.7134565937,
    'li_1':                1.1664710130072,
    'li_3':                0.9350306868856,
    'R_mid_edge':          8.2060589008712,
    'Btot_mid_edge':       4.2591971439040,
    'beta_t':              0.0175057596246,
    'beta_p':              0.4204336501823,
    'beta_n':              1.1764594851067,
    'cocos':               1,
    # --- Standard j_tor sampled at interior psi_N positions [A/m^2] ---
    # (j_tor_averaged = <Jt/R>/<1/R> from analytic Grad-Shafranov)
    'j_tor_psiN_10':       -1706295.2188907,
    'j_tor_psiN_25':       -1467240.5481605,
    'j_tor_psiN_50':        -947009.8179661,
    'j_tor_psiN_75':        -417067.3819351,
    'j_tor_psiN_90':        -144859.3170193,
    # --- Direct j_tor sampled at interior psi_N positions [A/m^2] ---
    # (j_tor_averaged_direct = -sigma_Bp * (p'*<R> + FF'*<1/R>/mu0) * (2pi)^exp_Bp
    # -- the "direct" Grad-Shafranov form used as the TokaMaker default)
    'j_tor_direct_psiN_10': -1702483.7399723,
    'j_tor_direct_psiN_25': -1459673.6530630,
    'j_tor_direct_psiN_50':  -940629.3261296,
    'j_tor_direct_psiN_75':  -418261.5724391,
    'j_tor_direct_psiN_90':  -147962.8902676,
    # --- q_profile sampled at interior psi_N positions ---
    'q_psiN_10':           -0.8382482256259,
    'q_psiN_25':           -0.8971299411392,
    'q_psiN_50':           -1.0948156933135,
    'q_psiN_75':           -1.5533089558791,
    'q_psiN_90':           -2.2515796211309,
    # NOTE: q at the LCFS (q_edge) is intentionally omitted -- for diverted
    # plasmas q diverges at the separatrix and different codes apply
    # different regularisations there.
}

# Per-key tolerance overrides for derived FSA quantities where the
# TokaMaker implementation uses a slightly different algorithm than OMFIT.
eqdsk_gfile_tol_dict = {
    # Geometric / topological quantities -- small algorithmic differences
    'kappa_edge':    2.0E-2,
    'a_edge':        2.0E-2,
    'vol_edge':      2.0E-2,
    'cxArea_edge':   2.0E-2,
    # Triangularity: OMFIT uses a 4-point parabolic fit at the extrema,
    # we use the standard midplane-point definition.  These can differ
    # by ~10% on shaped plasmas -- this is a definition difference, not
    # an accuracy problem.
    'delta_edge':    1.5E-1,
    # Internal inductance: sensitive to edge p' / FF' handling
    'li_1':          3.0E-2,
    # Betas: sensitive to pressure-integral details
    'beta_p':        3.0E-2,
    'beta_t':        2.0E-2,
    'beta_n':        2.0E-2,
    # j_tor is analytically computed from p' / FF' and FSA metrics --
    # alignment with OMFIT is ~0.004% RMS for standard j_tor and ~0.01%
    # for j_tor_direct.  Tight tolerance here catches regressions in the
    # FSA contour tracer or quadrature.
    'j_tor_psiN_10':         5.0E-3,
    'j_tor_psiN_25':         5.0E-3,
    'j_tor_psiN_50':         5.0E-3,
    'j_tor_psiN_75':         5.0E-3,
    'j_tor_psiN_90':         5.0E-3,
    'j_tor_direct_psiN_10':  5.0E-3,
    'j_tor_direct_psiN_25':  5.0E-3,
    'j_tor_direct_psiN_50':  5.0E-3,
    'j_tor_direct_psiN_75':  5.0E-3,
    'j_tor_direct_psiN_90':  5.0E-3,
    'q_psiN_10':             5.0E-3,
    'q_psiN_25':             5.0E-3,
    'q_psiN_50':             5.0E-3,
    'q_psiN_75':             5.0E-3,
...
#
# Expected values are generated by a one-time offline run of the OMFIT
# `OMFITpFile` reader on the same D3Dlike_Hmode_test.peqdsk file.  The
# TokaMaker reader was adapted from the OMFIT code, so raw profile
# quantities must match to floating-point precision.
# -----------------------------------------------------------------------
eqdsk_pfile_expected_dict = {
    # ne
    'ne_len':    514, 'ne_0':   0.55,     'ne_mid':  0.404252,
    'ne_end':    0.079838, 'ne_min': 0.079838, 'ne_max': 0.55,
    'ne_sum':    209.410982, 'ne_psinorm_end': 1.0,
    # te
    'te_len':    514, 'te_0':   2.5,      'te_mid':  1.769165,
    'te_end':    0.064254, 'te_min': 0.064254, 'te_max': 2.5,
    'te_sum':    877.048034, 'te_psinorm_end': 1.0,
    # ni
    'ni_len':    514, 'ni_0':   0.45,     'ni_mid':  0.331166,
    'ni_end':    0.065084, 'ni_min': 0.065084, 'ni_max': 0.45,
    'ni_sum':    173.613682, 'ni_psinorm_end': 1.0,
    # ti
    'ti_len':    514, 'ti_0':   5.0,      'ti_mid':  2.135399,
    'ti_end':    0.288054, 'ti_min': 0.288054, 'ti_max': 5.0,
    'ti_sum':    1237.5458, 'ti_psinorm_end': 1.0,
    # nz1
    'nz1_len':   514, 'nz1_0':  0.016667, 'nz1_mid': 0.012181,
    'nz1_end':   0.002459, 'nz1_min': 0.002459, 'nz1_max': 0.016667,
    'nz1_sum':   5.966222, 'nz1_psinorm_end': 1.0,
    # ptot
    'ptot_len':  514, 'ptot_0': 59.41405, 'ptot_mid': 23.205468,
    'ptot_end':  0.393912, 'ptot_min': 0.393912, 'ptot_max': 59.41405,
    'ptot_sum':  13691.080231, 'ptot_psinorm_end': 1.0,
    # Rotation decomposition
    'omgpp_len': 514, 'omgpp_0':  -12.279162, 'omgpp_mid': -2.101807,
    'omgpp_end': -5.43053, 'omgpp_sum': -2015.088177,
    'ommpp_len': 514, 'ommpp_0':  -37.901104, 'ommpp_mid': -14.776236,
    'ommpp_end': -63.888182, 'ommpp_sum': -11328.010237,
    'omepp_len': 514, 'omepp_0':  6.4078,     'omepp_mid': 9.095038,
    'omepp_end': 26.330202, 'omepp_sum': 6585.637141,
    'omgeb_len': 514, 'omgeb_0':  -12.279162, 'omgeb_mid': -2.101807,
    'omgeb_end': -5.43053, 'omgeb_sum': -2015.088177,
    'ommvb_len': 514, 'ommvb_0':  25.621942,  'ommvb_mid': 12.674429,
    'ommvb_end': 58.457652, 'ommvb_sum': 9312.922057,
    'omevb_len': 514, 'omevb_0':  -18.686962, 'omevb_mid': -11.196845,
    'omevb_end': -31.760732, 'omevb_sum': -8600.725304,
    'omghb_len': 514, 'omghb_mid': -1.141731, 'omghb_end': -2.36942,
    'omghb_sum': -172.732282,
    # Ion species block
    'N Z A_N_0': 6.0,
    'N Z A_Z_0': 6.0,
    'N Z A_A_0': 12.0,
