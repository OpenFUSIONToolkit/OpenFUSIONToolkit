...
def ThinCurr_setup(meshfile,run_type,direct_flag,freq=0.0,fr_limit=0,eta=10.0,use_aca=False,
    ...
    with open('oft.in','w+') as fid:
        fid.write(oft_in_template.format(meshfile,direct_flag,freq,fr_limit,coil_file_line,volt_file_line,L_svd_tol,B_svd_tol))
...
def test_eig_cyl(direct_flag):
    ...
    assert ThinCurr_setup("tw_test-cyl.h5",2,direct_flag)
    assert validate_eigs(eigs)
