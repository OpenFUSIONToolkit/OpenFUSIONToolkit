import os
import sys
import pytest
test_dir = os.path.abspath(os.path.dirname(__file__))
sys.path.append(os.path.abspath(os.path.join(test_dir, '..')))
from oft_testing import run_OFT, run_command, approx_list

# Basic template for input file
oft_in_template = """
&runtime_options
 ppn=1
 debug=0
 test_run=T
/

&mesh_options
 meshname='sphere'
 cad_type=0
 nlevels={1}
 nbase={0}
 grid_order={3}
/

&cubit_test_options
 test_surf={6}
/

&native_mesh_options
 filename='{2}.h5'
 reflect={4}
 ref_periodic={7}
 zstretch={5}
/
"""

# Common setup function and process handling
def cubit_setup(nbase, nlevels, prefix, grid_order=1,
                reflect='F', per_ns=-1, zstretch=1., test_2d='F'):
    #
    convert_cmd = ["OFT_convert_cubit.py", "--in_file={0}.g".format(prefix)]
    nproc = 1
    if nbase != nlevels:
        nproc = 2
    ref_periodic = 'F'
    if per_ns > 0:
        ref_periodic = 'T'
        convert_cmd += ["--periodic_nodeset={0}".format(per_ns)]
    #
    os.chdir(test_dir)
    with open('oft.in', 'w+') as fid:
        fid.write(oft_in_template.format(nbase, nlevels, prefix, grid_order, reflect, zstretch, test_2d, ref_periodic))
    # Run mesh conversion script
    outs, errs, errcode = run_command(" ".join(convert_cmd))
    if errcode != 0:
        print("FAILED: OFT_convert_cubit.py exited with non-zero error code!")
        print("========== STD OUTPUT ==========")
        print(outs.decode())
        print("========== ERR OUTPUT ==========")
        print(errs.decode())
        print("ERRCODE = {0}".format(errcode))
        print("========== END OUTPUT ==========")
        return False
    #
    return run_OFT("./test_cubit", nproc, 60)

# Validate results against expected values
def check_result(volume_test, area_test, nbp_test=None, nbe_test=None):
    retval = True
    fid = open('cubit.results','r')
    volume = float(fid.readline())
    if volume != volume_test:
        print("FAILED: Incorrect domain volume!")
        print("  Expected = {0}".format(volume_test))
        print("  Actual =   {0}".format(volume))
        retval = False
    area = float(fid.readline())
    if area != area_test:
        print("FAILED: Incorrect domain surface area!")
        print("  Expected = {0}".format(area_test))
        print("  Actual =   {0}".format(area))
        retval = False
    if nbp_test is not None:
        nbp = int(fid.readline())
        if nbp != nbp_test:
            print("FAILED: Incorrect number of boundary points!")
            print("  Expected = {0}".format(nbp_test))
            print("  Actual =   {0}".format(nbp))
            retval = False
    if nbe_test is not None:
        nbe = int(fid.readline())
        if nbe != nbe_test:
            print("FAILED: Incorrect number of boundary edges!")
            print("  Expected = {0}".format(nbe_test))
            print("  Actual =   {0}".format(nbe))
            retval = False
    return retval

#============================================================================
# Test runners for basic Cubit mesh
@pytest.mark.parametrize("top_lev", (1, 2))
def test_base(top_lev):
    volume_cubit = pytest.approx(3.9246, abs=1.E-4)
    area_cubit = pytest.approx(12.1320, abs=1.E-4)
    assert cubit_setup(1,top_lev,'sphere_tet4_test')
    assert check_result(volume_cubit, area_cubit)
@pytest.mark.parametrize("top_lev", (1, 2))
def test_hex_base(top_lev):
    volume_cubit = pytest.approx(3.91742, abs=1.E-4)
    area_cubit = pytest.approx(12.15673, abs=1.E-4)
    assert cubit_setup(1,top_lev,'sphere_hex8_test')
    assert check_result(volume_cubit, area_cubit)

#============================================================================
# Test runners for high order input meshes
@pytest.mark.parametrize("top_lev", (1, 2))
def test_tet10_quad(top_lev):
    volume_cubit = pytest.approx(4.18669, abs=1.E-4)
    area_cubit = pytest.approx(12.56224, abs=1.E-4)
    assert cubit_setup(1,top_lev,'sphere_tet10_test',grid_order=2)
    assert check_result(volume_cubit, area_cubit)
@pytest.mark.coverage
@pytest.mark.parametrize("top_lev", (2, 3))
@pytest.mark.parametrize("grid_order", (1, 2))
def test_tet10_1ref(top_lev,grid_order):
    volume_cubit = approx_list([4.12061, 4.18669], abs=1.E-4)
    area_cubit = approx_list([12.4552, 12.5622], abs=1.E-4)
    minlev = 4 - top_lev
    assert cubit_setup(minlev,top_lev,'sphere_tet10_test',grid_order=grid_order)
    assert check_result(volume_cubit[grid_order-1], area_cubit[grid_order-1])
@pytest.mark.parametrize("top_lev", (1, 2))
def test_hex27_quad(top_lev):
    volume_cubit = pytest.approx(4.18831, abs=1.E-4)
    area_cubit = pytest.approx(12.56542, abs=1.E-4)
    assert cubit_setup(1,top_lev,'sphere_hex27_test',grid_order=2)
    assert check_result(volume_cubit, area_cubit)
@pytest.mark.coverage
@pytest.mark.parametrize("top_lev", (2, 3))
@pytest.mark.parametrize("grid_order", (1, 2))
def test_hex27_1ref(top_lev,grid_order):
    volume_cubit = approx_list([4.11911, 4.18831], abs=1.E-4)
    area_cubit = approx_list([12.4621, 12.5654], abs=1.E-4)
    minlev = 4 - top_lev
    assert cubit_setup(minlev,top_lev,'sphere_hex27_test',grid_order=grid_order)
    assert check_result(volume_cubit[grid_order-1], area_cubit[grid_order-1])

#============================================================================
# Test runners for cut meshes
@pytest.mark.parametrize("top_lev", (1, 2))
def test_cut_base(top_lev):
    volume_cubit = pytest.approx(3.918291, abs=1.E-4)
    area_cubit = pytest.approx(12.12685, abs=1.E-4)
    assert cubit_setup(1,top_lev,'sphere_cut_test')
    assert check_result(volume_cubit, area_cubit)

#============================================================================
# Test runners for reflection
@pytest.mark.parametrize("top_lev", (1, 2))
def test_reflect_tri_base(top_lev):
    volume_cubit = pytest.approx(0.0, abs=1.E-4)
    area_cubit = pytest.approx(2.0, abs=1.E-4)
    assert cubit_setup(1,top_lev,'ref_tri3_test',reflect='T',test_2d='T')
    assert check_result(volume_cubit, area_cubit)
@pytest.mark.parametrize("top_lev", (1, 2))
def test_reflect_quad_base(top_lev):
    volume_cubit = pytest.approx(0.0, abs=1.E-4)
    area_cubit = pytest.approx(2.0, abs=1.E-4)
    assert cubit_setup(1,top_lev,'ref_quad4_test',reflect='T',test_2d='T')
    assert check_result(volume_cubit, area_cubit)
@pytest.mark.parametrize("top_lev", (1, 2))
def test_reflect_base(top_lev):
    volume_cubit = pytest.approx(3.02044, abs=1.E-4)
    area_cubit = pytest.approx(12.26295, abs=1.E-4)
    assert cubit_setup(1,top_lev,'ref_tet4_test',reflect='T')
    assert check_result(volume_cubit, area_cubit)
@pytest.mark.parametrize("top_lev", (1, 2))
def test_reflect_hex_base(top_lev):
    volume_cubit = pytest.approx(3.00000, abs=1.E-4)
    area_cubit = pytest.approx(12.21166, abs=1.E-4)
    assert cubit_setup(1,top_lev,'ref_hex8_test',reflect='T')
    assert check_result(volume_cubit, area_cubit)

#============================================================================
# Test runners for periodic reflection
@pytest.mark.parametrize("top_lev", (1, 2))
def test_perreflect_tri_base(top_lev):
    volume_cubit = pytest.approx(0.0, abs=1.E-4)
    area_cubit = pytest.approx(2.0, abs=1.E-4)
    assert cubit_setup(1,top_lev,'ref_tri3_test',reflect='T',per_ns=1,test_2d='T')
    assert check_result(volume_cubit, area_cubit, 80, 80)
@pytest.mark.parametrize("top_lev", (1, 2))
def test_perreflect_quad_base(top_lev):
    volume_cubit = pytest.approx(0.0, abs=1.E-4)
    area_cubit = pytest.approx(2.0, abs=1.E-4)
    assert cubit_setup(1,top_lev,'ref_quad4_test',reflect='T',per_ns=1,test_2d='T')
    assert check_result(volume_cubit, area_cubit, 80, 80)
@pytest.mark.parametrize("top_lev", (1, 2))
def test_perreflect_tet_base(top_lev):
    volume_cubit = pytest.approx(3.02044, abs=1.E-4)
    area_cubit = pytest.approx(6.22207, abs=1.E-4)
    assert cubit_setup(1,top_lev,'ref_tet4_test',reflect='T',per_ns=1)
    assert check_result(volume_cubit, area_cubit, 26, 78)
@pytest.mark.parametrize("top_lev", (1, 2))
def test_perreflect_hex_base(top_lev):
    volume_cubit = pytest.approx(3.00000, abs=1.E-4)
    area_cubit = pytest.approx(6.21166, abs=1.E-4)
    assert cubit_setup(1,top_lev,'ref_hex8_test',reflect='T',per_ns=1)
    assert check_result(volume_cubit, area_cubit, 24, 48)

#============================================================================
# Test runners for high order reflected meshes
@pytest.mark.parametrize("top_lev", (1, 2))
def test_tet10reflect_quad(top_lev):
    volume_cubit = pytest.approx(3.14123, abs=1.E-4)
    area_cubit =  pytest.approx(12.56531, abs=1.E-4)
    assert cubit_setup(1,top_lev,'ref_tet10_test',grid_order=2,reflect='T')
    assert check_result(volume_cubit, area_cubit)
@pytest.mark.coverage
@pytest.mark.parametrize("top_lev", (2, 3))
def test_tet10reflect_1ref(top_lev):
    volume_cubit = pytest.approx(3.11110, abs=1.E-4)
    area_cubit =  pytest.approx(12.48994, abs=1.E-4)
    minlev = 4 - top_lev
    assert cubit_setup(minlev,top_lev,'ref_tet10_test',reflect='T')
    assert check_result(volume_cubit, area_cubit)
@pytest.mark.parametrize("top_lev", (1, 2))
def test_hex27reflect_quad(top_lev):
    volume_cubit = pytest.approx(3.14110, abs=1.E-4)
    area_cubit =  pytest.approx(12.56491, abs=1.E-4)
    assert cubit_setup(1,top_lev,'ref_hex27_test',grid_order=2,reflect='T')
    assert check_result(volume_cubit, area_cubit)
@pytest.mark.coverage
@pytest.mark.parametrize("top_lev", (2, 3))
def test_hex27reflect_1ref(top_lev):
    volume_cubit = pytest.approx(3.10583, abs=1.E-4)
    area_cubit =  pytest.approx(12.47691, abs=1.E-4)
    minlev = 4 - top_lev
    assert cubit_setup(minlev,top_lev,'ref_hex27_test',reflect='T')
    assert check_result(volume_cubit, area_cubit)

#============================================================================
# Test runners for stretched meshes
@pytest.mark.parametrize("top_lev", (1, 2))
def test_stretch_base(top_lev):
    volume_cubit = pytest.approx(3.02044, abs=1.E-4)
    area_cubit =  pytest.approx(12.26295, abs=1.E-4)
    assert cubit_setup(1,top_lev,'ref_tet4_test',zstretch=2.)
    assert check_result(volume_cubit, area_cubit)

#============================================================================
# Test runners for surface meshes
@pytest.mark.parametrize("mesh_type", ("tri3", "tri6", "quad4", "quad9"))
@pytest.mark.parametrize("top_lev", (1, 2))
def test_surf_circle_base(mesh_type,top_lev):
    volume_cubit = pytest.approx(0.0, abs=1.E-4)
    area_cubit = pytest.approx(3.1403, abs=1.E-3)
    assert cubit_setup(1,top_lev,'circle_{0}_test'.format(mesh_type),test_2d='T')
    assert check_result(volume_cubit, area_cubit)
@pytest.mark.coverage
@pytest.mark.parametrize("mesh_type", ("tri3", "tri6", "quad4", "quad9"))
@pytest.mark.parametrize("levels", ((2, 2), (2, 3), (1, 3)))
def test_surf_circle_1ref(mesh_type,levels):
    volume_cubit = pytest.approx(0.0, abs=1.E-4)
    area_cubit = pytest.approx(3.1403, abs=1.E-3)
    assert cubit_setup(levels[0],levels[1],'circle_{0}_test'.format(mesh_type),test_2d='T')
    assert check_result(volume_cubit, area_cubit)
@pytest.mark.coverage
@pytest.mark.parametrize("mesh_type", ("tri6", "quad9"))
@pytest.mark.parametrize("top_lev", (1, 2))
def test_surf_circle_quad(mesh_type,top_lev):
    volume_cubit = pytest.approx(0.0, abs=1.E-4)
    area_cubit = pytest.approx(3.14160, abs=1.E-3)
    assert cubit_setup(1,top_lev,'circle_{0}_test'.format(mesh_type),test_2d='T',grid_order=2)
    assert check_result(volume_cubit, area_cubit)
@pytest.mark.parametrize("mesh_type", ("tri3", "quad4"))
@pytest.mark.parametrize("top_lev", (1, 2))
def test_surf_sphere_base(mesh_type,top_lev):
    volume_cubit = pytest.approx(0.0, abs=1.E-4)
    area_cubit = pytest.approx(12.5595, abs=1.E-3)
    assert cubit_setup(1,top_lev,'sphere_{0}_test'.format(mesh_type),test_2d='T')
    assert check_result(volume_cubit, area_cubit)
@pytest.mark.coverage
@pytest.mark.parametrize("mesh_type", ("tri3", "quad4"))
@pytest.mark.parametrize("levels", ((2, 2), (2, 3), (1, 3)))
def test_surf_sphere_1ref(mesh_type,levels):
    volume_cubit = pytest.approx(0.0, abs=1.E-4)
    area_cubit = pytest.approx(12.5595, abs=1.E-3)
    assert cubit_setup(levels[0],levels[1],'sphere_{0}_test'.format(mesh_type),test_2d='T')
    assert check_result(volume_cubit, area_cubit)
@pytest.mark.coverage
@pytest.mark.parametrize("mesh_type", ("tri6", "quad9"))
@pytest.mark.parametrize("top_lev", (1, 2))
def test_surf_sphere_quad(mesh_type,top_lev):
    volume_cubit = pytest.approx(0.0, abs=1.E-4)
    area_cubit = pytest.approx(12.5664, abs=1.E-3)
    assert cubit_setup(1,top_lev,'sphere_{0}_test'.format(mesh_type),test_2d='T',grid_order=2)
    assert check_result(volume_cubit, area_cubit)