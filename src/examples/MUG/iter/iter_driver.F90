...
INTEGER(i4), allocatable, dimension(:) :: reg_flag

LOGICAL :: pm=.FALSE.
LOGICAL :: use_mfnk=.FALSE.
LOGICAL :: success
CHARACTER(LEN=15) :: filename = 'none' !< Name of input file for mesh, fix later for variable length
...
ALLOCATE(reg_flag(mg_mesh%smesh%nreg))
DO j=1, SIZE(reg_flag)
  IF (j==1) reg_flag(1) = 1
  IF (j > 1 .AND. j<7) reg_flag(j) = 2
  IF (j >=7) reg_flag(j) = 3
END DO
!reg_flag(2) = 3
!reg_flag(3) = 3
!reg_flag(4) = 3
CALL mhd_sim%setup(mg_mesh,order, reg_flag)
...
n_eq = n0
T_eq = p_eq/(2*n0*elec_charge)

ALLOCATE(cell_dofs(ML_oft_blagrange%current_level%ne))
DO k=1,mg_mesh%smesh%nc
    IF(mg_mesh%smesh%reg(k) == 1) THEN ! if we are in a plasma region 
      call ML_oft_blagrange%current_level%ncdofs(k,cell_dofs) ! Get global index of local DOFs
      DO j=1, SIZE(cell_dofs)
        v_init(cell_dofs(j)) = 5.E3
      END DO
    END IF
END DO
...
eta_reg(3) = 1.d-3
eta_reg(4) = 1.d-3
eta_reg(5) = eta_reg(5)*mu0
eta_reg(6) = eta_reg(6)*mu0
!eta_reg(4) = 1.E-15
!eta_reg(6) = 1.E-15
