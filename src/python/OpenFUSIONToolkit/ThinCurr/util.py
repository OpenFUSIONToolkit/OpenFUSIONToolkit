...
class sensor_interface():
    ...
    def plot_inverse_2D_fourier_transform(self,t):
        ...
        plt.figure(figsize=(10,6))
        plt.title(fr"Reconstructing $B_{{norm}}$ from Fourier Transform on DCON surface with R = 1.85 at t = {t}")
    ...
    def plot_sensor_output(self,t):
        ...
        if hasattr(self,'hist_file') == False:
            ...
            plt.figure(figsize=(10,6))
            plt.title(rf"Magnetic Field on surfaces of toroidal planes with $R_0$ = {self.major_radius} at t = {t}")
    ...
    def plot_sensor_output_on_surface(self,t):
        ...
        for i, angle in zip([0,self.nphi//2],np.linspace(2*np.pi,0,self.nphi,endpoint=False)[[0,self.nphi//2]]):
            ...
            plt.xlabel('R')
            plt.ylabel('Z')
            plt.title(rf"Magnetic Field on the surface @ $\phi$ = {angle:.4f} with $R_0$ = {self.major_radius} at t = {t}")
            plt.gca().set_aspect('equal')
            plt.show()
