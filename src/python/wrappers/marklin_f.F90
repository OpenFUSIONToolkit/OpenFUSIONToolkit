...
CALL ML_hcurl_grad%vec_create(utmp)
CALL c_f_pointer(hmode_facs, facs_tmp, [taylor_nm])
DO i=1,taylor_nm
  CALL taylor_hffa(i,ML_oft_hcurl%level)%f%get_local(tmp)
...
CALL c_f_pointer(marklin_ptr,self)
ALLOCATE(interp_obj)
CALL ML_hcurl_grad%vec_create(interp_obj%uvac)
CALL c_f_pointer(vac_facs, vac_tmp, [taylor_nh])
DO i=1,taylor_nh
  CALL interp_obj%uvac%add(1.d0,vac_tmp(i),taylor_hvac(i,ML_hcurl_grad%level)%f)
END DO
