...
IF(self%boot_ops%initialized)THEN
  CALL hdf5_create_group(filename,'tokamaker/BOOT_OPS')
  ...
  CALL hdf5_write(self%boot_ops%djBS_tol,filename,'tokamaker/BOOT_OPS/DJBS_TOL')
...
IF(hdf5_field_exist(filename,'tokamaker/BOOT_OPS'))THEN
  ...
  CALL hdf5_read(self%boot_ops%djBS_tol,filename,'tokamaker/BOOT_OPS/DJBS_TOL',success=success)
