       ...
       DO m=1,quad%np
         ...
         DO jr=1,oft_blagrange%nce
           DO jc=1,oft_blagrange%nce
            ! Diffusion
            ! dx = jac_det*quad%wts(m) = dS
            jac_loc(1, 1)%m(jr,jc) = jac_loc(1, 1)%m(jr, jc) &
               + dn*basis_vals(jr)*jac_det*quad%wts(m) &
               + self%dt*basis_vals(jr)*dn*DOT_PRODUCT(basis_grads(:,jr), vel)*jac_det*quad%wts(m) & !dn*div(u)
               + self%dt*basis_vals(jr)*vel*basis_grads(:,jc)*jac_det*quad%wts(m) & ! u dot grad(dn)
               + self%dt*D*DOT_PRODUCT(basis_grads(:,jr),basis_grads(:,jc))*jac_det*quad%wts(m)
            jac_loc(1, 2)%m(jr,jc) = jac_loc(1, 2)%m(jr, jc) &
               + basis_vals(:,jr)*self%dt*n*div_vel*jac_det*quad%wts(m) & ! n*div(u)
               + basis_vals(:,jr)*self%dt*DOT_PRODUCT(dvel,dn) ! u dot grad(n)
            ! TODO: ^^delta_n != grad_n
            ! Temperature
            jac_loc(3, 1)%m(jr,jc) = jac_loc(3, 1)%m(jr, jc) &  
               + basis_vals(:, jr) * basis_grads(:, jc)/(gamma-1)*jac_det*quad%wts(m) &! delta_T
               + basis_vals(:, jr) * self%dt*DOT_PRODUCT(vel, basis_grads(:,jc))/(gamma-1)*jac_det*quad%wts(m) & ! nabla(dT)?
               - basis_vals(:, jr) * self%dt*k_boltz*dT*div_vel*jac_det*quad%wts(m) & ! dT != nabla(dT)
            ...
            jac_loc(3, 2)%m(jr,jc) = jac_loc(3, 2)%m(jr, jc) &  
               ...
               + basis_vals(:, jr) * self%dt*DOT_PRODUCT(basis_grads(:,jc), dT)/(gamma-1)*jac_det*quad%wts(m) & ! delta_u dot Delta_T
               - basis_vals(:, jr) * self%dt*k_boltz*T*SUM(basis_grads(:,jc))*jac_det*quad%wts(m) & ! div(delta u)
            ...
            jac_loc(3, 3)%m(jr,jc) = jac_loc(3, 3)%m(jr, jc) &  
               ...
               + self%dt * DOT_PRODUCT(dT,basis_grads(:,jr))*jac_det*quad%wts(m) & ! dT_Chi?
               - self%dt * basis_vals(:,jr)*DOT_PRODUCT(basis_grads(:,jc), dT)/n*jac_det*quad%wts(m) ! nabla(dT)?
