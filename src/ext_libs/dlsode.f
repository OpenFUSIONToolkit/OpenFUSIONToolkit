...
C***ROUTINES CALLED  DAXPY_LSODE, DSCAL_LSODE, IDAMAX_LSODE
C***REVISION HISTORY  (YYMMDD)
...
C
      DOUBLE PRECISION T
      INTEGER IDAMAX_LSODE,J,K,KP1,L,NM1
C
...
C
         L = IDAMAX_LSODE(N-K+1,A(K,K),1) + K - 1
         IPVT(K) = L
C
...
C
            T = -1.0D0/A(K,K)
            CALL DSCAL_LSODE(N-K,T,A(K+1,K),1)
C
...
C
            DO 30 J = KP1, N
               ...
               IF (L .EQ. K) GO TO 20
                  ...
                  A(K,J) = T
   20          CONTINUE
               CALL DAXPY_LSODE(N-K,T,A(K+1,K),1,A(K+1,J),1)
   30       CONTINUE
         GO TO 50
   40    CONTINUE
            INFO = K
...
C***REFERENCES  J. J. Dongarra, J. R. Bunch, C. B. Moler, and G. W.
C                 Stewart, LINPACK Users' Guide, SIAM, 1979.
C***ROUTINES CALLED  DAXPY_LSODE, DDOT_LSODE
C***REVISION HISTORY  (YYMMDD)
C   780814  DATE WRITTEN
C   890831  Modified array declarations.  (WRB)
C   890831  REVISION DATE from Version 3.2
...
C
         ...
         DO 20 K = 1, NM1
            ...
            IF (L .EQ. K) GO TO 10
               ...
               B(K) = T
   10       CONTINUE
            CALL DAXPY_LSODE(N-K,T,A(K+1,K),1,B(K+1),1)
   20    CONTINUE
   30    CONTINUE
C
C        NOW SOLVE  U*X = Y
C
         DO 40 KB = 1, N
            ...
            B(K) = B(K)/A(K,K)
            T = -B(K)
            CALL DAXPY_LSODE(K-1,T,A(1,K),1,B(1),1)
   40    CONTINUE
      GO TO 100
   50 CONTINUE
C
...
C
         DO 60 K = 1, N
            T = DDOT_LSODE(K-1,A(1,K),1,B(1),1)
            B(K) = (B(K) - T)/A(K,K)
   60    CONTINUE
C
...
C
         ...
         DO 80 KB = 1, NM1
            K = N - KB
            B(K) = B(K) + DDOT_LSODE(N-K,A(K+1,K),1,B(K+1),1)
            L = IPVT(K)
            IF (L .EQ. K) GO TO 70
               T = B(L)
               B(L) = B(K)
...
C
      DOUBLE PRECISION T
      INTEGER I,IDAMAX_LSODE,I0,J,JU,JZ,J0,J1,K,KP1,L,LM,M,MM,NM1
C
...
C
         LM = MIN(ML,N-K)
         L = IDAMAX_LSODE(LM+1,ABD(M,K),1) + M - 1
         IPVT(K) = L + K - M
C
C        ZERO PIVOT IMPLIES THIS COLUMN ALREADY TRIANGULARIZED
C
...
C
            T = -1.0D0/ABD(M,K)
            CALL DSCAL_LSODE(LM,T,ABD(M+1,K),1)
C
C           ROW ELIMINATION WITH COLUMN INDEXING
C
            ...
            DO 80 J = KP1, JU
               ...
               IF (L .EQ. MM) GO TO 70
                  ...
                  ABD(MM,J) = T
   70          CONTINUE
               CALL DAXPY_LSODE(LM,T,ABD(M+1,K),1,ABD(MM+1,J),1)
   80       CONTINUE
   90       CONTINUE
         GO TO 110
  100    CONTINUE
         ...
         IF (NM1 .LT. 1) GO TO 30
            DO 20 K = 1, NM1
               ...
               IF (L .EQ. K) GO TO 10
                  ...
                  B(K) = T
   10          CONTINUE
               CALL DAXPY_LSODE(LM,T,ABD(M+1,K),1,B(K+1),1)
   20       CONTINUE
   30    CONTINUE
C
C        NOW SOLVE  U*X = Y
C
         DO 40 KB = 1, N
            ...
            LB = K - LM
            T = -B(K)
            CALL DAXPY_LSODE(LM,T,ABD(LA,K),1,B(LB),1)
   40    CONTINUE
      GO TO 100
   50 CONTINUE
C
...
C
         DO 60 K = 1, N
            ...
            LA = M - LM
            LB = K - LM
            T = DDOT_LSODE(LM,ABD(LA,K),1,B(LB),1)
            B(K) = (B(K) - T)/ABD(M,K)
   60    CONTINUE
C
C        NOW SOLVE TRANS(L)*X = Y
C
         ...
         IF (NM1 .LT. 1) GO TO 90
            DO 80 KB = 1, NM1
               K = N - KB
               LM = MIN(ML,N-K)
               B(K) = B(K) + DDOT_LSODE(LM,ABD(M+1,K),1,B(K+1),1)
               L = IPVT(K)
               IF (L .EQ. K) GO TO 70
                  T = B(L)
                  B(L) = B(K)
  ...
  100 CONTINUE
      ...
      END
*DECK DAXPY_LSODE
      SUBROUTINE DAXPY_LSODE (N, DA, DX, INCX, DY, INCY)
C***BEGIN PROLOGUE  DAXPY_LSODE
C***PURPOSE  Compute a constant times a vector plus a vector.
C***CATEGORY  D1A7
C***TYPE      DOUBLE PRECISION (SAXPY-S, DAXPY_LSODE-D, CAXPY-C)
...
C   920501  Reformatted the REFERENCES section.  (WRB)
C***END PROLOGUE  DAXPY_LSODE
      DOUBLE PRECISION DX(*), DY(*), DA
C***FIRST EXECUTABLE STATEMENT  DAXPY_LSODE
...
C
   ...
   70 CONTINUE
      ...
      END
*DECK DDOT_LSODE
      DOUBLE PRECISION FUNCTION DDOT_LSODE (N, DX, INCX, DY, INCY)
C***BEGIN PROLOGUE  DDOT_LSODE
C***PURPOSE  Compute the inner product of two vectors.
C***CATEGORY  D1A4
C***TYPE      DOUBLE PRECISION (SDOT-S, DDOT_LSODE-D, CDOTU-C)
...
C
C     --Output--
C     DDOT_LSODE  double precision dot product (zero if N .LE. 0)
C
C     Returns the dot product of double precision DX and DY.
C     DDOT_LSODE = sum for I = 0 to N-1 of  DX(LX+I*INCX) * DY(LY+I*INCY),
C     where LX = 1 if INCX .GE. 0, else LX = 1+(1-N)*INCX, and LY is
C     defined in a similar way using INCY.
C
C***REFERENCES  C. L. Lawson, R. J. Hanson, D. R. Kincaid and F. T.
...
C   920310  Corrected definition of LX in DESCRIPTION.  (WRB)
C   920501  Reformatted the REFERENCES section.  (WRB)
C***END PROLOGUE  DDOT_LSODE
      DOUBLE PRECISION DX(*), DY(*)
C***FIRST EXECUTABLE STATEMENT  DDOT_LSODE
      DDOT_LSODE = 0.0D0
      IF (N .LE. 0) RETURN
      IF (INCX .EQ. INCY) IF (INCX-1) 5,20,60
C
...
C
    5 IX = 1
      ...
      IF (INCY .LT. 0) IY = (-N+1)*INCY + 1
      DO 10 I = 1,N
        DDOT_LSODE = DDOT_LSODE + DX(IX)*DY(IY)
        IX = IX + INCX
        IY = IY + INCY
   10 CONTINUE
      RETURN
...
C
   20 M = MOD(N,5)
      IF (M .EQ. 0) GO TO 40
      DO 30 I = 1,M
         DDOT_LSODE = DDOT_LSODE + DX(I)*DY(I)
   30 CONTINUE
      IF (N .LT. 5) RETURN
   40 MP1 = M + 1
      DO 50 I = MP1,N,5
      DDOT_LSODE = DDOT_LSODE + DX(I)*DY(I) + DX(I+1)*DY(I+1) + DX(I+2)*DY(I+2) +
...
C
   ...
   70 CONTINUE
      ...
      END
*DECK DSCAL_LSODE
      SUBROUTINE DSCAL_LSODE (N, DA, DX, INCX)
C***BEGIN PROLOGUE  DSCAL_LSODE
C***PURPOSE  Multiply a vector by a constant.
C***CATEGORY  D1A6
C***TYPE      DOUBLE PRECISION (SSCAL-S, DSCAL_LSODE-D, CSCAL-C)
...
C           (WRB)
C   920501  Reformatted the REFERENCES section.  (WRB)
C***END PROLOGUE  DSCAL_LSODE
      DOUBLE PRECISION DA, DX(*)
      INTEGER I, INCX, IX, M, MP1, N
C***FIRST EXECUTABLE STATEMENT  DSCAL_LSODE
      IF (N .LE. 0) RETURN
...
C
   ...
   50 CONTINUE
      ...
      END
*DECK IDAMAX_LSODE
      INTEGER FUNCTION IDAMAX_LSODE (N, DX, INCX)
C***BEGIN PROLOGUE  IDAMAX_LSODE
C***PURPOSE  Find the smallest index of that component of a vector
C            having the maximum magnitude.
C***CATEGORY  D1A2
C***TYPE      DOUBLE PRECISION (ISAMAX-S, IDAMAX_LSODE-D, ICAMAX-C)
...
C
C     --Output--
C   IDAMAX_LSODE  smallest index (zero if N .LE. 0)
C
C     Find smallest index of maximum magnitude of double precision DX.
C     IDAMAX_LSODE = first I, I = 1 to N, to maximize ABS(DX(IX+(I-1)*INCX)),
C     where IX = 1 if INCX .GE. 0, else IX = 1+(1-N)*INCX.
C
C***REFERENCES  C. L. Lawson, R. J. Hanson, D. R. Kincaid and F. T.
C                 Krogh, Basic linear algebra subprograms for Fortran
...
C           (WRB)
C   920501  Reformatted the REFERENCES section.  (WRB)
C***END PROLOGUE  IDAMAX_LSODE
      DOUBLE PRECISION DX(*), DMAX, XMAG
      INTEGER I, INCX, IX, N
C***FIRST EXECUTABLE STATEMENT  IDAMAX_LSODE
      IDAMAX_LSODE = 0
      IF (N .LE. 0) RETURN
      IDAMAX_LSODE = 1
      IF (N .EQ. 1) RETURN
